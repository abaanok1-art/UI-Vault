...
export default function Overview({ showToast }) {
  ...
  useEffect(() => {
    const savedStar = localStorage.getItem('uiStash_starred');
    if (savedStar === 'true') {
      setIsStarred(true);
...
.vault-cyber-btn:hover {
  ...
  const handleStar = () => {
    if (isStarred) {
      setStars(stars - 1);
      setIsStarred(false);
      localStorage.setItem('uiStash_starred', 'false');
      if (showToast) showToast('⭐ Star removed.');
    } else {
      setStars(stars + 1);
      setIsStarred(true);
      localStorage.setItem('uiStash_starred', 'true');
      if (showToast) showToast('🌟 Thank you for starring UI Stash!');
    }
  };
  ...
  return (
    <div className="overview-page-root">
      <div className="overview-hero">
        ...
        <h1 className="hero-title">Welcome to UI Stash</h1>
        <p className="hero-subtitle">
          UI Stash is an open-source library of dark-mode glassmorphic React components.
